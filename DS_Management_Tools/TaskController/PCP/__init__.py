...
def start_controller(tid, input_data, session={}, controller=None, block=True, method='', sample=False):
    ...
    if method == 'GeneratorMode':
        with open(os.path.join(_dir, 'PCPBatteryGenerator.json')) as f:
            metatask_data = json.load(f)
    elif method == 'MotorMode':
        with open(os.path.join(_dir, 'PCPBatteryMotor.json')) as f:
            metatask_data = json.load(f)
    elif method == 'Test':
        with open(os.path.join(_dir, 'PCPTest.json')) as f:
            metatask_data = json.load(f)
...
if __name__ == "__main__":
    ...
    result = start_controller(tid=100, input_data=input_data, method='Test')
