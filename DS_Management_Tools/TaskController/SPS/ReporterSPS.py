...
class ReporterSPS(brp):
    ...
    def report(self, method=''):
        ...
        name = 'result-%s.mat' % (str(time.time()))
        print 'savemat-----START'
        savemat(os.path.join(__conf__['outputPath'], 'mat', name), mdict=convert(self.__mat), do_compression=True)

        print 'savemat-----FINISH'
        # super(ReporterSPS, self).report()
        self.__mat = {}
        self.successful = []
        self.failed = []
        return name
    ...
    def get_allResults(self, mapping, tasks):
        ...
        for task in tasks:
            ...
            # cell['result']['power'] = float(cell['result']['torque']) *
            #                           cell['simulationParameterSet']['RPM'] / 60.0 * 2.0 * numpy.pi
    ...
    def get_Dymola2DSet(self, mapping, metatask):
        ...
        subdict['Name'] = 'AP1'
        subdict['modelName'] = metatask['taskGenerator']['arguments'][
            'constant']['parameterOfFunction']['testArguments']['modelName']
        constant_parameters = metatask['taskGenerator']['arguments'][
            'constant']['parameterOfFunction']['testArguments']['parameters']
    ...
    def get_function_argments(self, metatask, sp_map):
        ...
        return [self.get_constant_cell(self.get_key_from_value(sp_map, key), func_arg[key]['maxValue'])
                for key in func_arg.keys()]
