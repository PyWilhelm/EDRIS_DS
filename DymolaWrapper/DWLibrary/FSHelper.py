...
class FSHelper(object):
    ...
    def get_dymosim_files():
        ...
        return [FSHelper.add_extension_exe("dymosim"), "buildlog.txt", "dsin.txt"]

    @staticmethod
    def get_dymosim_path(model_name, build_directory='.', is_jenkins=False):
        """Get get the path of the according dymosim.exe"""
        if is_jenkins:
            file_path = FSHelper.add_extension_exe(model_name.replace('.', os.sep) + '_dymosim')
        else:
            file_path = 'dymosim.exe'
    ...
    @staticmethod
    def file_path_with_prefix(model_name, file_name, build_directory='.'):
    ...
    def get_relative_directory_model(model_name):
        ...
        directory_path = os.path.split(model_name.replace('.', os.sep))[0]
