...
class EnvConfiguration():
        ...
        if not 'workingDirectory' in self.__directory:
            if os.environ.get('J_DYMOLA_WD') != None:
                ...
                self.__directory['workingDirectory'] = os.path.abspath(
                                self.__env_configuration["workingDirectory"])
    ...
    def publish_dir(self):
        if not 'publishDirectory' in self.__directory:
            publish_direction = os.path.abspath(self.__env_configuration["publishDirectory"])
    ...
    def tsts_json(self):
        if not 'testsJson' in self.__directory:
            self.__directory['testsJson'] = JsonReader(self.__env_configuration["testsJsonFilePath"]).read()
            #FIXME: maybe it is better to put it outside of the file
            self.__directory['testsJson'] = generate_new_tests(self.__env_configuration["testsJsonFilePath"], 
                                                               self.__env_configuration["librariesLocation"])
        return self.__directory['testsJson']

    @property
    def libraries_location(self):
        return self.__env_configuration['librariesLocation']

    @property
    def dymola_path(self):
        return self.__env_configuration["dymolaPath"]


    @property
    def dymola_interface_path(self):
        return self.__env_configuration["dymolaInterfacePath"]
